...
The following example shows testing a one-shot publisher (<<#reference-datataskpublisher,URLSession.dataTaskPublisher>> in this case) using expectation, and expecting the data to flow without an error.
...
The publisher is invoked by attaching the <<reference.adoc#reference-sink,sink>> subscriber to it.
