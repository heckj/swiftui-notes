...
[NOTE]
====
While you can also use <<reference.adoc#rerefence-breakpoint>> and <<reference.adoc#rerefence-breakpointOnError>> operators to break into a debugger (as shown in <<#patterns-debugging-breakpoint>>), the handleEvents() operator with closures allows you to set breakpoints within Xcode.
