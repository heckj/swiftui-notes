...
I have removed some of the other extraneous print statements, which I also placed in <<reference#reference-sink,sink>> closures to see final results.
...
If you want to be more selective with what you identify and print, or if you need to process the data passing through for it to be used more meaningfully, then you look at the <<reference#reference-handleevents,handleEvents>> operator. More detail on how to use this opeartor for debugging is in  <<patterns#patterns-debugging-handleevents,Debugging pipelines with the handleEvents operator>>.
