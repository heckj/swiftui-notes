...
If we used an optional `String?` instead of an array of `String[]`, the optional does not trigger some of the pipeline when it is nill, and we always want a result value - even an empty value - to come from the pipeline.
====
