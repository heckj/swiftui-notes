...
The subscribers (created with <<reference.adoc#reference-assign>> and <<reference.adoc#reference-sink,Sink>>) are stored as AnyCancellable variables on the ViewController instance.
...
If the pipeline from a <<reference.adoc#reference-published>> variable terminates in a <<reference.adoc#reference-sink,Sink>> that accepts an Error failure type, the sink will send a termination signal if an error occures, which stops the pipeline from further processing even when the variable is updated.
====
