...
The example provided here takes input from the user (a username) and attempts a lookup against an API (GitHub's API, which is nicely documented) to provide data about that user.
...
3. We have a another subscriber `repositoryCountSubscriber` attached to `$githubUserData` that pulls the repository count off the github user data object and assigns it as the count to be displayed.
4. We have a final subscriber `avatarViewSubscriber` attached to `$githubUserData` that attempts to retrieve the image associated with the user's avatar for display.
...
The three subscribers (one created with <<reference.adoc#reference-assign>> and the other two with <<reference.adoc#reference-sink>>) are also stored as AnyCancellable variables on the ViewController instance.
...
private struct GithubAPIUser: Decodable { <1>
...
private struct GithubAPI { <2>
    ...
    static func retrieveGithubUser(username: String)
        -> AnyPublisher<[GithubAPIUser], Never> {
        ...
        let publisher = URLSession.shared.dataTaskPublisher(for:
            URL(string: assembledURL)!)
//            .handleEvents(receiveSubscription: { _ in <4>
...
class ViewController: UIViewController {
    ...
    @Published private var githubUserData: [GithubAPIUser] = [] <3>

    // publisher reference for this is $username, of type <String, Never>
    var myBackgroundQueue: DispatchQueue = DispatchQueue(
        label: "viewControllerBackgroundQueue")
    ...
    override func viewDidLoad() {
        ...
        let usernameSub = $username
            ...
            .map { username -> AnyPublisher<[GithubAPIUser], Never> in <6>
        ...
        usernameSubscriber = AnyCancellable(usernameSub) <7>
        ...
        let avatarViewSub = $githubUserData
            ...
            .map { userData -> AnyPublisher<UIImage, Never> in
                guard let firstUser = userData.first else { <8>
                ...
                return URLSession.shared.dataTaskPublisher(
                    for: URL(string: firstUser.avatar_url)!)
            ...
            // .assign(to: \.image, on: self.githubAvatarImageView) <10>
...
<3> I first created the pipelines to return an optional GithubAPIUser instance, but found that there wasn't a convenient way to propogate "nil" objects on failure conditions.
...
<4> The handleEvents operator wasn't originally in the viewController, and I left it here as an example.
This particular example shows how you can leverage handleEvents to trigger events to update UI elements. The commented out example shows a reference to an activityIndicator being turned on or off, depending on the event being processed.
<5> The <<reference.adoc#reference-throttle>> and
<<reference.adoc#reference-removeDuplicates>> are specifically there to collapse events from the changing username so that API requests aren't made on rapidly changing values.
...
<6> map is used similiarly to flatMap in error handling here, with the pipeline requiring the <<reference.adoc#reference-switchToLatest>> operator to provide the intended correct result at the end.
The API object returns a publisher, which this map is invoking. This doesn't return the value from the call, but the calling publisher itself.
...
<7> The AnyCancellable is used to convert the cancellable object created by the sink subscriber into an AnyCancellable instance.
<8> The avatar pipeline has two instances locations where it might provide alternate values.
The first is when an empty list of GithubAPIUser is provided and we want to "erase" any previously displayed avatar image.
<9> The second is when the API call to retrieve the an image has failed.
<10> I tried to use an <<reference.adoc#reference-assign>> subscriber to set the image into the UIImageView, but we getting an error.
Replacing it with  <<reference.adoc#reference-sink>> works fine.
