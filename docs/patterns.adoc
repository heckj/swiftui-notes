...
For example, if you are working with SwiftUI and the you want to use <<#reference-assign,assign>> to set the `isEnabled` property on a button, the subscriber will have a few requirements:

. the subcriber should match the type output of `<Bool>`, `<Never>`
. the subscriber should be called on the main thread

With a publisher that can throw an error (such as <<#reference-datataskpublisher,URLSession.dataTaskPublisher>>), you need to construct a pipeline to convert the output type, but also handle the error within the pipeline to match a failure type of `<Never>`.

How you handle the errors within a pipeline is very dependent on how the pipeline is working.
If the pipeline is set up to return a single result and terminate, continue to <<#patterns-oneshot-error-handling,Using catch to handle errors in a one-shot pipeline>>.
If the pipeline is set up to continually update, the error handling needs to be a little more complex.
Jump ahead to <<#patterns-continual-error-handling,Using flatMap with catch to handle errors>>.
...
This might include returning data from a publisher created with <<reference.adoc#reference-just,Just>> or <<reference.adoc#reference-fail,Fail>>, or something more complex using <<reference.adoc#reference-future,Future>>.
