...
[WARNING]
====
This sample is originally from the WWDC session, but the API and example has evolved with the beta's of Combine since that presentation.
`tryCatch` in particular appears to have been removed.
I suspect `mapError` is an appropriate replacement, but that detail has yet to be confirmed.
====
