...
<4> <<reference#reference-combinelatest,combineLatets>> takes two publishers and merges them into a single pipeline with an output type that is the combined values of each of the upstream publishers.
...
<6> The two vaildation pipelines are combined with <<reference#reference-combinelatest,combineLatest>>, and the output of those checked and merged into a single tuple output.
