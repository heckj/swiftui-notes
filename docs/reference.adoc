...
Be aware that a Future creates and invokes its closure to do the asynchronous request *at the time of creation*, not when the publisher receives a demand request. 
This can be extremely counter-intuitive.

[WARNING]
====
A side effect of how Future works means that as of Combine as released in iOS 13.2, Future is incompatible with the retry operator.
...
If you want to request repeated results from a Future publisher, you will need to either create one each time you have demand.
This can be done using either by creating a new publisher with flatMap, or perhaps with Deferred.
