...
Throttle is akin to the <<#reference-debounce>> operator in that it collapses values, but throttle defaults to returning the first value set within the time window specified rather than the last value, which debounce returns.
