...
Because of this protocol/lifecycle expectation, it is common to arrange pipelines from these publishers that have an error type of `<Never>` and do all error handling within the pipelines.
...
[WARNING]
====
As of the beta3 release of Combine with the updated operating systems, Published doesn't always trigger when a struct is the holding variable, but it works within a class instance.
...
=== publisher property on KVO references
