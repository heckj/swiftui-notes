...
`Record` acts very similiarly to <<reference-sequence>> if you want to publish a sequence of values and then send a `.finished` completion.
...
In making a publisher conform to the https://developer.apple.com/documentation/combine/connectablepublisher[`ConnectablePublisher`] protocol.

 the publisher will have a new method `connect` that returns a Cancellable (similiar to <<reference-sink,sink>> or <<reference-assign,assign>>).

A connectable publisher has two means that creating the connection between subscribers and the publisher are under more explicit control.
There are two methods that are exposed to handle this connectivity: `connect` and `autoconnect`.

With `connect`, the method will be under imperative control.
...
With a connectable publisher, in addition to setting up the subscription you would need to explicitly invoke `connect()` on the publisher to allow it to accept subscriptions and do any relevant processing.
...
The above code won't activate the subscription, and in turn show any results, without explicitly invoking `connect()` on the publisher:
...
If you have a connectable publisher, and want it to act like the other publishers and automatically connect upon a subscription, you can use `autoconnect()` to have it operate in that fashion.

One of the primary uses of having a connectable publisher is to coordinate pipeline creation with <<reference-multicast,multicast>> which shares a single subscription request to multiple subscribers without repeating the upstream processing.
...
The two mechahisms allow you to choose which you want for the needs of your code.
...
Similiar to the <<reference-containswhere,containsWhere>> operator, this operator is provided with a closure.
...
Instead it uses a trigger that activates once, and propogates all values after it is activated until the upstream publisher finishes.
...
Further demand will be propogated upward to `secondPublisher`.
...
While it returns `true` the values are propogated to the subscriber.
...
This will select the fourth item published from the upstream publisher to propogate.
...
This will select the third and fourth item published from the upstream publisher to propogate.
...
`Zip` works very similiarly to <<reference#reference-combinelatest,combineLatest>>, connecting two upstream publishers and providing the output of those publishers as a single pipeline with a tuple output type composed of the types of the upstream publishers.
...
`switchToLatest` is operates similarly to <<reference#reference-flatmap,flatMap>>, taking in a publisher instance and returning its value (or values).
...
A commonly desirede operation is to decode some provided data, so Combine provides the `decode` operator suited to that task.
...
When creating using multicast, you either provide a <<reference-subject>> (with the parameter `subject) or create a <<reference-subject>> inline in a closure.
...
Some publishers (such as <<reference#reference-datataskpublisher,URLSession.dataTaskPublisher>>) have complex internals that will run on alternative queues based on their configuration, and will be relatively uneffected by `subscribe`.
...
* <<patterns#pattern-cascading-update-interface,Cascading multiple UI updates, including a network request>>

__Details__::

When chaining operators together, the resulting type signature accumulates all the various types.
This can get complicated quite quickly, and can provide an unnessecarily complex signature for an API.
