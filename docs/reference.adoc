...
Similiar to publishers having a type-erased struct <<reference-anypublisher,AnyPublisher>> to expose publishers through an API, subscribers have an equivalent: <<reference-anycancellable,AnyCancellable>>. This is often used with `sink` to convert the resulting type into AnyCancellable.
