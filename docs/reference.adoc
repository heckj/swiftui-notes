...
__icon:apple[] docs__:: https://developer.apple.com/documentation/combine/publisher/3204758-subscribe[`subscribe`]
...
Subscribe assigns a scheduler to any preceding pipeline invocations, and is often used to invoke a pipeline on a background thread or queue.
It can be used to request a publisher on a background thread, and then use <<#reference-receive>> to transfer data to another thread (such as the main runloop) appropriate for updating user interface objects.
...
* <<patterns.adoc#patterns-assign-subscriber>> shows an example of using assign to set an a boolean property on a UI element.
* tests illustrating assign: `UsingCombineTests/????.swift`
...
* tests illustrating a sink: `UsingCombineTests/SinkSubscriberTests.swift`
