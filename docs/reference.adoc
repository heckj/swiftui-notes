...
A connectable publisher has an explicit mechaninism for enabling when a subscription and the flow of demand from subscribers will be allowed to the publisher.
...
Both of these methods return a `Cancellable` (similiar to <<reference-sink,sink>> or <<reference-assign,assign>>).
