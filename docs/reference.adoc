...
A variation of the <<reference-trycontains>> operator which allows the closure to throw an error.
...
If you want a set number of values from the front of the stream you can also use <<reference-prefix>> or it's variants: <<reference-prefixwhile,prefixWhile>> and <<reference-tryprefixwhile,tryPrefixWhile>>.
...
If you want to use this mechanism, but with a publisher as the trigger instead of a closure, use the <<reference-dropwuntiloutput,dropUntilOutput>> operator.
...
The <<reference-preppend>> operator is often used with single or sequence values that don't error.
